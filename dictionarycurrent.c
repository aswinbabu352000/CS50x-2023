...
unsigned int hash(const char *word)
{
    ...
    // determine the x according to first alphabet of the word
    ...
    {
        x = (((toupper(word[0]) - 'A') + 1) * 26) + (toupper(word[1]) - 'A');
    }
    else
    {
        x = ((((toupper(word[0] - 'A') + 1) * 26) * 26) * 26) + (((toupper(word[1] - 'A') + 1) * 26) * 26) + (((toupper(word[2]) - 'A') + 1) * 26) + (toupper(word[3]) - 'A');
    }
